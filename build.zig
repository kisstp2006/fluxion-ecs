...
pub fn build(b: *std.Build) void {
    ...
    const example_mod = b.createModule(.{
        ...
        .target = target,
        .optimize = optimize,
        .imports = &.{.{ .name = "fluxion_ecs", .module = mod }},
    });
    const example = b.addExecutable(.{ .name = "fluxion-ecs-demo", .root_module = example_mod });
    ...
    test_step.dependOn(&b.addRunArtifact(b.addTest(.{
        .name = "fluxion-ecs-demo-tests",
        .root_module = example_mod,
    })).step);

    // zig build web -> zig-out/web
    //
    // The same world, in a browser: no threads, so the scheduler has no
    // workers and the page's frame callback drives it. Built for
    ...
    const web_mod = b.createModule(.{
        ...
        .imports = &.{.{ .name = "fluxion_ecs", .module = wasm_mod }},
    });
    const web = b.addExecutable(.{ .name = "fluxion-ecs-web", .root_module = web_mod });
    web.entry = .disabled;
    web.rdynamic = true;
    ...
    // no-thread path should fail here, not in a browser later.
    test_step.dependOn(web_step);

    // zig build docs -> zig-out/docs
    const docs_lib = b.addLibrary(.{ .name = "fluxion-ecs", .root_module = mod });
...
fn module(
    ...
    return b.addModule(b.fmt("fluxion_ecs{s}", .{
        ...
        .imports = &.{
            ...
            .{ .name = "fluxion_mem", .module = memory.module("fluxion_mem") },
